...
#               : alat Aircrack-ng secara otomatis.
# + Pembuat     : Rofidoang03
# ---------------------------------------------------------------
...
function instal_depedensi(){
    ...
    chmod +x src/aircrack-ng.sh

    echo ""
    echo -e "${p}[${c}info${p}] Semua depedensi yang diperlukan berhasil diinstal.${r}"
    sleep 1
    echo -e "${p}[${c}info${p}] Aircrack-ng berhasil diinstal.${r}"
    sleep 1
    echo -e "${p}[${c}info${p}] Untuk menjalankan ketikkan perintah ${h}./aircrack-ng.sh${r}"
    sleep 1
    echo ""
    exit 0
}
...
function instal_aircrack(){
    clear    
    read -p $'\e[1;37mApakah Anda ingin menginstal alat Aircrack-ng [iya/tidak]: ' nanya
